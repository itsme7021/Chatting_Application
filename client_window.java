...
public class client_window implements ActionListener {
    ...
    client_window()
    {
        ...
        //adding a timer object that will help change the active now status to typing whenever a key is pressed and back to
        // active now after 2seconds of wait when the key is released
        ...
        //after 2 seconds of delay it will go back to active now
        ...
        //adding key listener that is whenever a key is pressed set the text to typing and when released to active now
    ...
    public static void main(String[] args) {
        new client_window().f1.setVisible(true);
        try {
            s3 = new Socket("127.0.0.1" , 600109);  //for the local host this is the ip address and we knwo the port we want this to connect
                                                            // so we write exactly that only
            d1 = new DataInputStream(s3.getInputStream());
            d2 = new DataOutputStream(s3.getOutputStream());
